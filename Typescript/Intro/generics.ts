...
// Keyof Constraint
function extractAndExplain<T extends object, U extends keyof T>(obj: T,key: U){
    return obj[key];
}
...
class StorageData<T extends string | boolean | number>{
    private data:T[] = [];
    addItem(item:T){
        this.data.push(item);
    }
...
const textData = new StorageData<string>();
textData.addItem('Try');
console.log(textData.getItems());
const numData = new StorageData<number | boolean>(); // Can be flexible with types
